...
```py title="伪代码"
fn md5_kugou_v4(data: bytes, table: str):
    md5_final = [] # 预期大小 = 31，实际使用后 30 项。
    md5_digest = md5(data)
    indexes = [
    ...
    for index in indexes:
        md5_final << md5_digest[index]

    result = [] # 预期大小 = 4 * (md5_final.len() - 1) * (key.len() - 1)

    # 枚举所有 data/table 的元素，跳过第一项。
    for i in 1..data.len():
        for j in 1..md5_final.len():
...
fn set_slot_key(slot_key):
    ...
    key1 = md5_kugou_v4(key1, str_slot_key_table)

fn set_file_key(file_key):
    key2 = md5_kugou_v4(file_key, str_file_key_table)
